...
switch (basename($_SERVER['PHP_SELF'])) {
  ...
  case 'Peserta_Seminar.php': $b='active'; break;
...
<div class="ui stackable top fixed container inverted borderless blue menu">
    ...
    <div class="ui dropdown item">Peserta Terdaftar <i class="dropdown icon"></i>
      <div class="menu">
        <?php
        // TODO: tambah menu untuk menampilkan peserta dari semua seminar$listSeminar=$conn->query("SELECT * FROM seminar")->fetchAll();
        if(sizeof($listSeminar)>0){?>
          <a href="<?php echo 'Peserta.php' ?>" class="<?php if(basename($_SERVER['PHP_SELF'])=='Peserta.php' && empty($_GET['id'])) echo 'active'; ?> item">Seluruh Peserta</a>
          <?phpforeach($listSeminar as $row) {
            if($b!='' && !empty($_GET['id'])){
              if($row['id']==$_GET['id']) $bid='active';
            } ?>
            <a href="<?php echo "Peserta_Seminar.php?id=".$row['id']; ?>" class="<?php echo $bid; $bid='' ?> item"><?php echo $row['nama'];?></a>
          <?php } }else{ ?>
