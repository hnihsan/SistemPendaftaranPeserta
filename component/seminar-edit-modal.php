...
<form class="ui form" action="controller/SeminarController.php" method="post">
  ...
  <div class="two fields">
    <div class="field">
      ...
      <input id="kuota" type="number" name="kuota" required="" max="300" value="<?php echo $sql[0]['kuota'] ?>">
    </div>
    <div class="field">
      <label for="email">Narasumber</label>
        <input id="narasumber" type="text" name="narasumber" required="" onkeypress="return (event.charCode >= 65 && event.charCode <= 90) || (event.charCode >= 96 && event.charCode <= 122) || (event.charCode >= 32 && event.charCode <= 32)" value="<?php echo $sql[0]['narasumber'] ?>">
      </div>
    </div>
  </div>
  <div class="actions">
